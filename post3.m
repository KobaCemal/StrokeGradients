...
mean_stroke_right=mean(corrmats_corrected_reduced(201:end,201:end,subjects.lesion_side==1),3,"omitnan");%mean_stroke_right(isinf(mean_stroke_right))=1;
mean_stroke_left=mean(corrmats_corrected_reduced(1:200,1:200,subjects.lesion_side==0),3,"omitnan");%mean_stroke_left(isinf(mean_stroke_left))=1;
...
for i=1:size(corrmats_corrected_reduced,3)
    disp(i)
    ...
    m=corrmats_corrected_reduced(201:end,201:end,i);
    m(isnan(m))=1;
    ...
    m=corrmats_corrected_reduced(1:200,1:200,i);
    m(isnan(m))=1;
...
gradients_stroke_left=gradients_subjectwise_left_aligned(:,:,subjects.lesion_side==0);sl=mean(gradients_stroke_left,3);
gradients_stroke_right=gradients_subjectwise_right_aligned(:,:,subjects.lesion_side==1);sr=mean(gradients_stroke_right,3);
...
plot_hemispheres([mean_left_distance(:,1:5);mean_right_distance(:,1:5)] , {surf_lh,surf_rh}, ...
    'parcellation',labeling.schaefer_400);

plot_hemispheres( [cl(:,2) sl(:,2) mean_left_distance(:,2);cr(:,2) sr(:,2) mean_right_distance(:,2)], {surf_lh,surf_rh}, ...
    'parcellation',labeling.schaefer_400,'labeltext',{'2nd gradient of control','2nd gradient of stroke', 'Mean euclidian difference'});
